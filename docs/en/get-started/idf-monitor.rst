...
The IDF Monitor tool is a Python program which runs when the ``idf.py monitor`` target is invoked in IDF.
...
The keyboard shortcut ``Ctrl-T Ctrl-F`` will pause IDF Monitor, run the ``idf.py flash`` target, then resume IDF Monitor. Any changed source files will be recompiled before re-flashing.

The keyboard shortcut ``Ctrl-T Ctrl-A`` will pause IDF Monitor, run the ``idf.py app-flash`` target, then resume IDF Monitor. This is similar to ``idf.py flash``, but only the main app is compiled and reflashed.
...
- Occasionally when "make" exits, it may stall for up to 30 seconds before IDF Monitor resumes.
